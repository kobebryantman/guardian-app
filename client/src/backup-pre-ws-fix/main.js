...
ipcMain.handle('bind-student', async (_, { joinCode, studentId, studentName }) => {
  if (!joinCode || !studentId) {
    return { ok: false, msg: '缺少接入码或学号' };
  }

  if (!remoteConfig.serverUrl) {
    return { ok: false, msg: '请先填写服务器地址并保存' };
  }

  try {
    // 根据 serverUrl 协议选择 http/https 模块
    const urlObj = new URL(remoteConfig.serverUrl);
    const isHttps = urlObj.protocol === 'wss:';
    const transport = isHttps ? require('https') : require('http');
    const hostname = urlObj.hostname;
    const port = urlObj.port || (isHttps ? '443' : '80');

    return new Promise((resolve) => {
      const req = transport.request({ hostname, port, path: '/api/student/bind', method: 'POST', headers: { 'Content-Type': 'application/json' }, rejectUnauthorized: false, timeout: 5000 }, (res) => {
      let d = '';
      res.on('data', c => d += c);
      res.on('end', () => {
        try {
          ...
          if (r.ok) {
            ...
            remoteConfig.studentName = r.name || studentName;
            remoteConfig.joinCode = joinCode;
            saveRemoteConfig(remoteConfig);
            if (remote) remote.bindStudent(r.studentId, remoteConfig.studentName, joinCode);
            resolve({ ok: true, studentId: r.studentId, studentName: r.name || studentName });
          } else {
    ...
    req.on('timeout', () => { req.destroy(); resolve({ ok: false, msg: '连接超时，请检查服务器地址' }); });
    req.write(JSON.stringify({ joinCode, studentId, name: studentName, hostname: os.hostname() }));
    req.end();
  });
  } catch (e) {
    return { ok: false, msg: '服务器地址格式错误' };
  }
});
